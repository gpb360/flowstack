    ...
    >
      {(isSelected || isHovered) && !block.locked && (
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-0 flex items-center gap-2 bg-blue-500 text-white px-3 py-1 rounded-md text-sm font-medium">
            <span>Divider</span>
          </div>
        </div>
      )}
      ...
      <hr
        style={{
          border: 'none',
          borderTop: `${block.props.thickness || '1px'} ${borderStyle} ${block.props.color || '#e5e7eb'}`,
